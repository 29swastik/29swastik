<h1 align="center">Hi, I'm Swastik K 👋</h1>

<p align="center">
  <strong>Senior AI Engineer</strong> • GenAI • RAG • Multi-Agent Systems • Search & Retrieval
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/swastik-k-173b84151/">
    <img alt="LinkedIn" src="https://img.shields.io/badge/LinkedIn-Swastik%20K-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://29swastik.github.io/">
    <img alt="Portfolio" src="https://img.shields.io/badge/Portfolio-Visit-111827?style=for-the-badge&logo=firefox&logoColor=white"/>
  </a>
  <a href="mailto:kswastik29@gmail.com">
    <img alt="Email" src="https://img.shields.io/badge/Email-Contact-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

---

## About Me

Senior AI Engineer with 5+ years of experience building and scaling **production-grade AI systems**.

I work at the intersection of:
- Generative AI (LLMs, Agents)
- Retrieval Systems (RAG, Hybrid Search)
- Applied Machine Learning
- Production Infrastructure & MLOps

My focus is simple: **build AI systems that actually work in production** — not just demos.

---

## What I Work On

- Designing **end-to-end RAG pipelines** (multimodal, hybrid retrieval)
- Building **multi-agent systems** using LangGraph & ReAct
- Developing **search and ranking systems** (BM25 + dense embedding + reranking)
- Productionizing ML systems with **evaluation, monitoring, and observability**
- Optimizing systems for **latency, relevance, and scale**

---

## Tech Stack

<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/SQL-003B57?style=flat-square"/>
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white"/>
  <img src="https://img.shields.io/badge/Kafka-231F20?style=flat-square&logo=apachekafka&logoColor=white"/>
  <img src="https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square"/>
  <img src="https://img.shields.io/badge/LangGraph-111827?style=flat-square"/>
  <img src="https://img.shields.io/badge/RAG-4B5563?style=flat-square"/>
  <img src="https://img.shields.io/badge/BM25-6B7280?style=flat-square"/>
  <img src="https://img.shields.io/badge/Elasticsearch-005571?style=flat-square&logo=elasticsearch&logoColor=white"/>
  <img src="https://img.shields.io/badge/Qdrant-DC2626?style=flat-square"/>
  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white"/>
  <img src="https://img.shields.io/badge/XGBoost-EC6B23?style=flat-square"/>
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white"/>
  <img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white"/>
  <img src="https://img.shields.io/badge/MLflow-0194E2?style=flat-square"/>
  <img src="https://img.shields.io/badge/DVC-13ADC7?style=flat-square"/>
</p>

---

## Experience Highlights

### Infineon Technologies — Staff Specialist AI Engineer
- Built a **multi-agent technical support system** → +60% answer relevancy  
- Designed a **multimodal RAG pipeline** → +70% improvement  
- Productionized ML pipelines with MLflow & DVC  

### HerKey — Senior Search Specialist
- Built **hybrid retrieval system** → 6× faster candidate search  
- Improved **precision@10 by 25%** using ML-based reranking  
- Delivered APIs with **<100ms latency**  

### Engati — Senior Software Engineer
- Built conversational AI systems → +40% engagement  
- Developed **Text-to-SQL system (85% accuracy)**  
- Shipped LLM-based chatbot for enterprise use cases  

---

## Open Source

### 🔗 LangChain Skills Adapter
👉 https://github.com/29swastik/langchain-skills-adapter

- Plug-and-play skill integration for LangChain agents  
- Simplifies building agent with skills  

---

## Focus Areas

- Generative AI in production  
- Retrieval-Augmented Generation (RAG)  
- Multi-Agent Systems  
- Search & Ranking Systems  
- ML System Design  
- Evaluation & Observability  

> Building AI systems that are not just intelligent, but reliable, measurable, and production-ready.
